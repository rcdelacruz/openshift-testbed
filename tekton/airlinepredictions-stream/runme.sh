...
oc new-project airlineprediction-stream-dev
oc new-project airlineprediction-stream-staging
oc new-project airlineprediction-stream-build

# create regcred secret
#oc create secret generic regcred --from-file=.dockerconfigjson="${QUAY_USERNAME}-airlinepredictionstream-auth.json" --type=kubernetes.io/dockerconfigjson
oc create -f secrets/
...
echo https://github.com/rcdelacruz/airlineprediction-stream.git

echo
echo Clone your forked repo so you can push to it
echo git clone https://github.com/${GITHUB_USERNAME}/airlineprediction-stream
